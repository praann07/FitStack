...
import { range } from '@/lib/validate'
...
/**
 * Shown right after a first-time OTP verification (authStore status
 * 'needs_onboarding'). Collects the same fields the old single-screen
 * register form did, minus email/password -- Supabase Auth already has
 * those. See authService.completeOnboarding for the writes this triggers.
 */
export function OnboardingPage() {
  ...
  async function submit(e: React.FormEvent) {
    ...
    if (fullName.trim().length < 2) errs.full_name = 'Full name must be at least 2 characters'
  ...
  return (
    <AuthLayout>
      <div className="animate-scale-in rounded-2xl border border-line bg-surface p-6 shadow-[var(--shadow-pop)] sm:p-7">
        <h1 className="text-xl font-bold tracking-tight text-ink">Complete your profile</h1>
        <p className="mt-1 text-[13.5px] text-ink-muted">
          One account for training, nutrition and progress. Targets start from a standard baseline
          and become adaptive as you log.
        </p>
